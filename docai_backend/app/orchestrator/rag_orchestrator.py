...
class RAGOrchestrator:
    def __init__(self):
        ...
        self.generator_agent = GeneratorAgent()
         # Use the imported verifier_agent instance and expose it for tests/mocking
        from app.agents.verifier_agent import verifier_agent as _verifier
        self.verifier_agent = _verifier
    ...
    async def handle_query(
        ...
        logger.info(f"Language: {lang or 'en'}")
        logger.info(f"Mode: {'Streaming' if stream else 'Sync'}")
        logger.info(f"Parameters: top_k={top_k}, return_top={return_top}")
        ...
        if stream:
            ...
            # When stream=False, generator_agent.generate returns an awaitable coroutine
            answer = await self.generator_agent.generate(
                question, candidates["chunks"], stream=False, request_id=request_id
            )
            # Clean up the answer by removing any XML tags that might have been generated
            import re
    ...
    async def _stream_response(self, candidates: Dict, question: str, request_id: str, retrieval_ms: int) -> AsyncGenerator[str, None]:
        ...
        # Clean up the full answer by removing any XML tags that might have been generated
        import re
