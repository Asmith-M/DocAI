...
def get_cached_candidates(document_id: str, question: str) -> Dict[str, Any]:
    key = f"candidates:{document_id}:{question}"
...
def get_cached_answer(document_id: str, question: str) -> Dict[str, Any]:
    key = f"answer:{document_id}:{question}"
    return rag_cache.get(key)

def set_cached_answer(document_id: str, question: str, answer_data: Dict[str, Any]):
    key = f"answer:{document_id}:{question}"
    rag_cache[key] = answer_data

def clear_cache(document_id: str = None):
    if document_id:
        # Remove all keys starting with document_id
        keys_to_remove = [k for k in rag_cache.keys() if k.startswith(f"candidates:{document_id}:") or k.startswith(f"answer:{document_id}:")]
