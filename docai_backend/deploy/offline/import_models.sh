...
download_with_progress() {
    ...
    curl -L -o "$output" "$url" || {
        echo -e "${RED}ERROR: Failed to download $output${NC}"
        return 1
    }
}
...
if [ $free_space_gb -lt 10 ]; then
    echo -e "${YELLOW}WARNING: Less than 10GB free space. Some models may not fit.${NC}"
fi
...
if ! curl -s http://localhost:11434/api/tags >/dev/null 2>&1; then
    ...
    echo -e "${GREEN}SUCCESS: Ollama service started${NC}"
else
    echo -e "${GREEN}SUCCESS: Ollama service already running${NC}"
fi
...
try:
    ...
    print('SUCCESS: Successfully downloaded embedding model')
...
echo -e "${BLUE}Configuring offline environment...${NC}"
...
EOF
    ...
    echo -e "${GREEN}SUCCESS: .env file already exists${NC}"
fi
...
# Final summary
echo ""
echo -e "${GREEN}SUCCESS: Model import completed successfully!${NC}"
echo ""
